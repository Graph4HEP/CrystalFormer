...
from crystalformer.src.attention_parallel import MultiHeadAttention
...
def make_transformer(key, Nf, Kx, Kl, n_max, h0_size, num_layers, num_heads, key_size, model_size, embed_size, atom_types, wyck_types, dropout_rate, widening_factor=4, sigmamin=1e-3):
    ...
    def network(G, XYZ, A, W, M, is_train):
        ...
        for _ in range(num_layers):
            ...
            h_attn = hk.dropout(hk.next_rng_key(), dropout_rate, h_attn) * is_train + h_attn*(1-is_train)
            h = h + h_attn
            ...
            h_dense = hk.dropout(hk.next_rng_key(), dropout_rate, h_dense) * is_train + h_dense * (1-is_train)
            h = h + h_dense

...
def getEnergy(path, cifs, thre, label):
    os.makedirs(path, exist_ok=True)
    relaxer = Relaxer("mace")
    calculator = relaxer.calculator
    for cif in tqdm.tqdm(cifs):
        try:
            structure = Structure.from_str(cif, fmt="cif")
        except:
            continue
        result_temp = relaxer.relax(structure, fmax=0.05, steps=20, traj_file=None)
        atoms = AseAtomsAdaptor.get_atoms(Structure.from_dict(result_temp["final_structure"]))
        atoms.calc = relaxer.calculator 
        print('energy:', get_e_form_per_atom(atoms, atoms.get_potential_energy()), ', force:', np.max(abs(atoms.get_forces())))
        if(get_e_form_per_atom(atoms, atoms.get_potential_energy())<thre and get_e_form_per_atom(atoms, atoms.get_potential_energy())>-10 and np.max(abs(atoms.get_forces())) < 10):
